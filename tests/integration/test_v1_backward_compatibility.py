...
class TestV1BackwardCompatibility:
    ...
    def test_v1_list_endpoint_legacy_format(self, client):
        ...
        if response.status_code == 200 and len(response.json()) > 0:
            ...
            assert "descripcion" in product, (
                "Legacy format should have descripcion field"
            )
    ...
    def test_v1_endpoints_use_service_dependency(self):
        ...
        for endpoint in v1_endpoints:
            ...
            assert "service" in params, (
                f"{endpoint.__name__} should have service parameter for DI"
            )

            # Check it's injected via Depends
            service_param = params["service"]
            assert service_param.default is not inspect.Parameter.empty, (
    ...
    def test_v1_list_optional_parameters(self, client):
        ...
        assert buscar_param.default is not inspect.Parameter.empty, (
    ...
    def test_v1_endpoint_limit_range(self, client):
        ...
        assert response.status_code != 422, (
            "V1 should accept limit=500 for backward compatibility"
        )
        ...
        for endpoint_sig in v1_endpoints:
            if endpoint_sig in source:
                ...
                next_decorator = source.find(
                    "@router.get", start_idx + len(endpoint_sig)
                )
                if next_decorator == -1:
