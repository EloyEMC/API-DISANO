...
class TestBC3Compatibility:
    ...
    def test_bc3_fields_present_in_v2_response(self, client):
        ...
        if len(data) > 0:
            ...
            assert product["bc3_descripcion_corta"] is not None, (
                "bc3_descripcion_corta should not be None"
            )

            assert product["bc3_product_type"] is not None, (
                "bc3_product_type should not be None"
            )

            assert product["bc3_descripcion_completa"] is not None, (
    ...
    def test_bc3_performance_benchmark_v2_list(self, client):
        ...
        assert elapsed < 0.5, (
            f"V2 list endpoint should respond in < 500ms, took {elapsed:.3f}s"
        )
    ...
    def test_bc3_performance_benchmark_v2_detail(self, client):
        ...
        assert response.status_code in [200, 404], (
            "V2 detail endpoint should return 200 or 404"
        )

        assert elapsed < 0.5, (
            f"V2 detail endpoint should respond in < 500ms, took {elapsed:.3f}s"
        )
    ...
    def test_bc3_performance_benchmark_v1_list(self, client):
        ...
        assert elapsed < 0.5, (
            f"V1 list endpoint should respond in < 500ms, took {elapsed:.3f}s"
        )
    ...
    def test_bc3_performance_benchmark_v1_detail(self, client):
        ...
        # Should be 200 or 404 (not found)
        assert response.status_code in [200, 404], (
            "V1 detail endpoint should return 200 or 404"
        )

        assert elapsed < 0.5, (
            f"V1 detail endpoint should respond in < 500ms, took {elapsed:.3f}s"
        )
            ...
            if len(data) > 0:
                ...
                for field in required_fields:
                    assert field in product, (
                        f"BC3 Suite (legacy) requires field: {field}"
                    )
