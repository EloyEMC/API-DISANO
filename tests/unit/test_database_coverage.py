"""
Tests Unitarios - Database (Aumenta coverage sin Settings import)
========================================================
...
def test_database_module_exists():
    ...
    assert os.path.exists("app/database.py"), "app/database.py no existe"


def test_database_module_has_required_functions():
...
def test_database_test_db_has_correct_structure():
    ...
    for col in required_columns:
        assert col in columns, f"Columna {col} no encontrada. Columnas: {list(columns)}"

    connection.close()


def test_database_test_db_has_products():
    """Verificar que testing.db tiene productos."""
    test_db = Path("testing/testing.db")
    connection = sqlite3.connect(test_db)
    ...
    assert count == 8288, f"Base de datos testing debe tener 8288 productos, tiene {count}"
