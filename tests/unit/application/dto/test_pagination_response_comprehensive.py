...
def test_pagination_metadata_exact_page_count() -> None:
    ...
    metadata = PaginationMetadata.from_query(
        total_items=100, current_page=2, per_page=20
    )
...
def test_pagination_metadata_large_page_numbers() -> None:
    ...
    metadata = PaginationMetadata.from_query(
        total_items=10000, current_page=100, per_page=50
    )
...
def test_pagination_metadata_model_dump() -> None:
    ...
    metadata = PaginationMetadata.from_query(
        total_items=50, current_page=2, per_page=10
    )
...
def test_paginated_response_multiple_filters() -> None:
    ...
    metadata = PaginationMetadata.from_query(
        total_items=25, current_page=1, per_page=10
    )
...
def test_paginated_response_various_item_types() -> None:
    ...
    response_dict = PaginatedResponseDTO(
        items=[{"id": 1}, {"id": 2}], pagination=metadata
    )
...
def test_paginated_response_model_dump() -> None:
    """Test model_dump works correctly."""
    metadata = PaginationMetadata.from_query(
        total_items=20, current_page=1, per_page=10
    )
...
def test_pagination_metadata_first_and_last_page_flags() -> None:
    ...
    # First page
    metadata_first = PaginationMetadata.from_query(
        total_items=100, current_page=1, per_page=20
    )
    assert metadata_first.has_next is True
    assert metadata_first.has_previous is False

    # Middle page
    metadata_middle = PaginationMetadata.from_query(
    ...
    # Last page
    metadata_last = PaginationMetadata.from_query(
        total_items=100, current_page=5, per_page=20
    )
...
def test_pagination_metadata_per_page_variations() -> None:
    ...
    for per_page in per_page_values:
        metadata = PaginationMetadata.from_query(
            total_items=100, current_page=1, per_page=per_page
        )
